...
# <pep8 compliant>
"""Status-bar + MU-panel progress for long .mu import.
...
# ``Loading texture 12/52…`` / ``Building object 3/40…`` → same console phase.
...
class MuProgressSession:
    """Window-manager progress bar + workspace status text + throttled redraw."""

    def __init__(self, context, total=100, title="MU Import"):
        self.context = context
        self.total = max(int(total), 1)
        self.title = (title or "MU Import").strip() or "MU Import"
        self.value = 0
        self._last_draw = 0.0
        self._last_status = ""
    ...
    def _maybe_redraw(self, force=False):
        ...
        if (not force) and (now - self._last_draw) < 0.12:
            return
        self._last_draw = now
        # DRAW_WIN_SWAP paints status bar + MU panel during a blocking
...
def draw_mu_panel_progress(layout):
    """Native Blender progress bar for the MU N-panel. True if drawn."""
    s = _active
    if s is None:
        return False
...
@contextmanager
def mu_progress_bar(context, total=100, title="MU Import"):
    session = MuProgressSession(context, total=total, title=title)
    session.begin()
    try:
        yield session
    finally:
...
def mu_tick(value=None, *, fraction=None, text=None, force=False):
    s = _active
    if s is None:
        return
...
def mu_tick_items(index, count, start, end, text=None, every=None, force=False):
    s = _active
    if s is None:
        return
